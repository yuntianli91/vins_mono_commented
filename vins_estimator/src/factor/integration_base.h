...
/**
 * @brief class for IMU preintegration
 * 
 */
class IntegrationBase
{
  public:
    ...
    /**
     * @brief Construct a new Integration Base object fot IMU integration between two frames [k,k+1]
     * 
     * @param _acc_0 : initial acceleration at k
     * @param _gyr_0 : initial angular rate at k
     * @param _linearized_ba : guess of acceleration bias at k
     * @param _linearized_bg : guess of acceleration bias at k
     */
    ...
    /**
     * @brief performing midPoint integration for IMU preintegrations and Jacobian
     * 
     * @param _dt : time step of integration intervel
     * @param _acc_0 : last acceleration measurement of IMU (t)
     * @param _gyr_0 : last gyro measurement of IMU (t)
     * @param _acc_1 : current acceleration measurement of IMU (t+1)
     * @param _gyr_1 : current gyro measurement of IMU (t+1)
     * @param delta_p : IMU preintegration of position at t
     * @param delta_q : IMU preintegration of rotation quaternion from b_k to b_t (corresponding R_{b_t}^{b_k})
     * @param delta_v : IMU ptrintegration of velocity at t
     * @param linearized_ba : accelerometer bias at t
     * @param linearized_bg : gyro bias at t
     * @param result_delta_p : IMU preintegration of position at t+1
     * @param result_delta_q : IMU preintegration of rotation quaternion from b_k to b_{t+1} (corresponding R_{b_{t+1}}^{b_k})
     * @param result_delta_v : IMU pretegration of velocity at t+1
     * @param result_linearized_ba : accelerometer bias at t+1,  equal to accelerometer bias at t
     * @param result_linearized_bg : gyro bias at t+1, equal to gyro bias at t
     * @param update_jacobian : whether to update jacobian matrix or not
     */
